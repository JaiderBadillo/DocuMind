...
from ..core.database import get_db, log_audit, sync_user_to_seed_db
...
def register_user(user: UserCreate):
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM users WHERE LOWER(TRIM(email)) = ?", (clean_email,))
        if cursor.fetchone():
        ...
        cursor.execute(
            ...
            (clean_email, clean_name, hashed_pwd, "USER")
        )
        ...
        log_audit("USER_REGISTER", "SUCCESS", f"Usuario {clean_email} registrado exitosamente", user_id=user_id, conn=conn)
        
    # Sync to persistent seed DB so the user is not lost if the container reloads
    sync_user_to_seed_db(user_id)
...
def login_user(creds: UserLogin):
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id, email, full_name, password_hash, role, created_at FROM users WHERE LOWER(TRIM(email)) = ?", (clean_email,))
        row = cursor.fetchone()
        
        if not row or not verify_password(creds.password, row["password_hash"]):
            log_audit("USER_LOGIN_FAILED", "WARNING", f"Intento fallido de login para: {clean_email}", conn=conn)
            raise HTTPException(status_code=401, detail="Credenciales incorrectas")
            
        token = create_access_token({"sub": row["email"], "user_id": row["id"], "role": row["role"]})
        log_audit("USER_LOGIN", "SUCCESS", f"Sesión iniciada por {clean_email}", user_id=row["id"], conn=conn)
